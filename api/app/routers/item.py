...
async def create_item(item: ItemCreate, db=Depends(get_db)):
    ...
    """
    if (await db.execute(select(ItemModel).where(ItemModel.name == item.name))).scalar():
        raise HTTPException(detail="Item already exists.", status_code=status.HTTP_400_BAD_REQUEST)
    item_object = ItemModel(**item.dict())
    db.add(item_object)
    await db.commit()
    return item_object
...
async def get_item(item_id: int, db=Depends(get_db)) -> Item:
    ...
    """
    item_in_db = (await db.execute(select(ItemModel).where(ItemModel.id == item_id))).scalar()
    return item_in_db
